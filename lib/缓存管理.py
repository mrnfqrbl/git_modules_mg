...
# 过滤逻辑
# ─────────────────────────────────────────────
def 过滤待处理列表(清单: 模块清单, 缓存: 处理缓存, 子模块锁定表: dict[str, str] = None) -> list[str]:
    """
    返回需要本次处理的 URL 列表。
    规则：
        - 未在缓存中 → 处理
        - 标记为"强制重迁" → 处理
        - 子模块当前的锁定 commit 与缓存中记录的上一次原始 commit 不符 → 处理
        - 否则跳过
    """
    待处理 = []
    for url in 清单.获取所有url():
        ...
        # 增量检测：如果本地子模块指针改变，则需要重新迁移以对齐最新版本
        if 子模块锁定表 and url in 子模块锁定表:
            当前锁定 = 子模块锁定表[url]
            最新记录 = 缓存.最新记录(url)
            if 最新记录 and 最新记录.get("原始commit") != 当前锁定:
                待处理.append(url)
                continue
